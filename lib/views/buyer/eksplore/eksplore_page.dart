...
class _ExplorePageState extends State<ExplorePage> {
  // ──────────────────────────────────────────
  // DATA PRODUK
  // TODO(backend): Ganti dengan data dari ProductController
  // ──────────────────────────────────────────
  ...
  // Filter state (temporary — saat di dalam bottom sheet)
  Set<String> _tempSelectedCategories = {};
  String? _tempSelectedPriceRange;
  int? _tempSelectedRating;

  // Filter state (applied — setelah tombol Terapkan ditekan)
  ...
  // ──────────────────────────────────────────
  // FILTER LOGIC
  // TODO(backend): Ganti dengan ProductController.applyFilters()
  // ──────────────────────────────────────────
  void _applyFilter() {
    ...
    setState(() {
      _filteredProducts = _products.where((p) {
        ...
        // Category filter
        final matchCategory = _appliedCategories.isEmpty ||
            _appliedCategories.contains(p['category']);

        // Price filter
        bool matchPrice = true;
        if (_appliedPriceRange != null) {
          final range = _priceRangeOptions.firstWhere(
        ...
        // Rating filter
        final matchRating =
            _appliedRating == null || (p['rating'] as double) >= _appliedRating!;
  ...
  int get _activeFilterCount =>
      (_appliedCategories.isNotEmpty ? 1 : 0) +
      (_appliedPriceRange != null ? 1 : 0) +
      (_appliedRating != null ? 1 : 0);
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: const Padding(
            ...
            child: Icon(Icons.close, size: 18, color: Color(0xFF9098B1)),
          ),
        ),
      ],
    );
  ...
  void _showFilterSheet() {
    // Salin applied ke temp saat sheet dibuka
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _FilterSheet(
        ...
        initialRating: _tempSelectedRating,
        onApply: (categories, priceRange, rating) {
          setState(() {
            _appliedCategories = categories;
            _appliedPriceRange = priceRange;
...
// ============================================================
// FILTER SHEET - Widget terpisah agar state-nya bersih
// ============================================================
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Column(
                      children: widget.priceRangeOptions.map((range) {
                        final bool isSelected =
                            _selectedPriceRange == range['label'];
                        return GestureDetector(
                          onTap: () => setState(() {
                            _selectedPriceRange =
                                isSelected ? null : range['label'];
                          }),
                          child: AnimatedContainer(
                            duration: const Duration(milliseconds: 150),
                            ...
                            decoration: BoxDecoration(
                              color: isSelected
                                  ? const Color(0xFF6B7FD7).withOpacity(0.08)
                    ...
                    Column(
                      children: List.generate(5, (i) {
                        final int starValue = 5 - i; // 5 → 1 dari atas ke bawah
                        final bool isSelected = _selectedRating == starValue;
                        return GestureDetector(
                          onTap: () => setState(() {
                            _selectedRating =
                                isSelected ? null : starValue;
                          }),
                          child: AnimatedContainer(
                            duration: const Duration(milliseconds: 150),
                            margin: const EdgeInsets.only(bottom: 8),
                  ...
                  children: [
                    ...
                    Expanded(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(
                          _hasSelection
                              ? 'Terapkan Filter ($_activeFilterCount)'
                              : 'Terapkan Filter',
                        ),
                      ),
                    ),
